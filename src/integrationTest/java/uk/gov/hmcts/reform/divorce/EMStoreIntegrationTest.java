...
public class EMStoreIntegrationTest extends IntegrationTest {
    ...
    private static final String DN_INPUT_JSON = "dn.json";
    ...
    @Test
    public void givenAllTheRightParameters_whenGenerateDNWithDocmosis_thenGeneratedPDFShouldBeStoredInEMStore()
        throws Exception {
        String requestBody = loadJson(DN_INPUT_JSON);
        //check PDF is generated
        Response response = callDivDocumentGenerator(requestBody);
        Assert.assertEquals(HttpStatus.OK.value(), response.getStatusCode());
        String documentUri = response.getBody().jsonPath().get(DOCUMENT_URL_KEY);
